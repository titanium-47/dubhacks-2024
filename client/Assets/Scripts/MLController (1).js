    ...
    for (var dy = 0; dy < ny; dy++) {
      for (var dx = 0; dx < nx; dx++) {
        for (var da = 0; da < anchors.length; da++) {
          var idx =
            dy * nx * anchors.length * step +
            dx * anchors.length * step +
            da * step;
          ...
          if (conf > scoreThreshold && anchors[i]) {
...
function onUpdate(eventData) {
  ...
  var result = DetectionHelpers.nms(
    boxes,
    scores,
    scoreThreshold,
    iouThreshold
...
function getClassLabel(index) {
  return classSettings[index].label
    ? classSettings[index].label
    : "class_" + index;
}
