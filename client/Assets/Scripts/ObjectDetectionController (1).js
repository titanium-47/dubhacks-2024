...
function instantiateObjects(origin, count) {
  ...
  if (origin.getComponent("ScreenTransform") == null) {
    if (!script.camera) {
      print(
        "Warning, please set Camera input to calculate world position of instantiated object"
      );
      return arr;
    } else {
      ...
      depth = origin
        .getTransform()
        .getWorldPosition()
        .distance(cameraTransform.getWorldPosition());
    }
  }
...
function updateDetectionBoxes(detections) {
  for (var i = 0; i < detections.length; i++) {
    if (i < maxCount) {
      ...
      //if api available
      if (
        trackletObjects[i].scriptComponent &&
        trackletObjects[i].scriptComponent.updateDetection
      ) {
...
function updateDetectionBoxesWithMatching(detections) {
  ...
  for (var i = 0; i < detections.length; i++) {
    ...
    for (var k = 0; k < num_active; k++) {
      ...
      var iou = DetectionHelpers.iou(
        detections[i].bbox,
        trackletObjects[active_tracklets[k]].detection.bbox
      );
      if (iou > best_iou) {
        best_iou = iou;
        best_tracklet_idx = k;
    ...
    if (best_tracklet_idx == -1 || best_iou < matchThreshold) {
      ...
      if (
        trackletObjects[temp_idx].scriptComponent &&
        trackletObjects[temp_idx].scriptComponent.updateDetection
      ) {
  ...
  for (var l = 0; l < maxCount; l++) {
    if (!trackletObjects[l].updated) {
      if (
        trackletObjects[l].active &&
        trackletObjects[l].lostFrames < lostFramesThreshold
      ) {
      ...
      if (num_new > 0) {
        ...
        if (trackletObjects[l].scriptComponent != null) {
          ...
          if (trackletObjects[l].scriptComponent.updateDetection) {
            trackletObjects[l].scriptComponent.updateDetection(
              new_tracklets[first_new]
            );
          }
        }
...
function updatePosition(tracklet) {
  ...
  if (tracklet.screenTransform) {
    ...
    if (tracklet.updated) {
      newAnchors = lerpRect(
        tracklet.screenTransform.anchors,
        newAnchors,
        lerpCoef
      );
    }
    ...
    var newPos = camera.screenSpaceToWorldSpace(
      tracklet.detection.getScreenPos(),
      depth
    );
    if (tracklet.updated) {
      newPos = vec3.lerp(
        tracklet.transform.getWorldPosition(),
        newPos,
        lerpCoef
      );
    }
...
function checkInputs() {
  if (!origin) {
    print(
      "Error, Please set the object you would like to instantiate for each detected object"
    );
    return false;
  }
  if (!mlController || !mlController.onDetectionsUpdated) {
    print(
      "Error, MLController script input is not set or wrong script referenced"
    );
    return false;
  }
